defmodule PicselloWeb.Helpers do
  ...
  def gallery_url(%{client_link_hash: hash}),
    do: Routes.gallery_client_show_url(Endpoint, :show, hash)

  def order_url(%{client_link_hash: hash}, order),
    do: Routes.gallery_client_order_url(Endpoint, :show, hash, Order.number(order))
