defmodule PicselloWeb.GalleryLive.Photos.Main do
  ...
  @impl true
  def handle_info(
    {:save, %{preview_photo_id: preview_photo_id, frame_id: frame_id}}, %{assigns: %{gallery: gallery}} = socket) do

    socket
    |> assign(products: Galleries.products(gallery))
    |> close_modal()
    |> put_flash(:photo_success, "Product preview successfully updated")
    |> noreply
  end
