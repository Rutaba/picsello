defmodule PicselloWeb.Shared.MultiSelect do
  @moduledoc """
  ...
  attr :search_on, :boolean, default: false
  attr :hide_tags, :boolean, default: false
  ...
  @css %{
    ...
    body:
      "-mt-[4px] p-2 ml-0 z-5 outline-none flex flex-col border rounded-lg absolute bottom-full shadow-md" <>
        ((@use_alpinejs && "") || " hidden"),
    filter:
    ...
    options:
      "overflow-auto max-h-48 pt-1 pl-1 scrollbar scrollbar-thumb-zinc-400 scrollbar-track-zinc-200 dark:scrollbar-thumb-gray-700 dark:scrollbar-track-gray-900 text-base-300",
    option_label: "flex text-sm font-medium text-gray-900 dark:text-gray-300 place-items-center",
    option_input:
      "rounded w-4 h-4 mr-2 dark:checked:bg-primary-500 border border-gray-300 focus:ring-primary-500 dark:focus:ring-primary-600 dark:ring-offset-gray-800 focus:ring-1 dark:bg-gray-700 dark:border-gray-600 transition duration-200 checkbox",
    colors:
  ...
  def update(%{options: options} = params, socket) do
    ...
    assigns =
      assigns
      |> assign(:selected_count, Enum.count(filter_checked_options(options)))
  ...
  def handle_event("wrapped", %{"value" => wrapped, "count" => count}, socket) do
    IO.puts(
      "Wrapped: #{wrapped}, count: #{count}, cur=#{socket.assigns.cur_shown}, max=#{socket.assigns.max_shown}"
    )
