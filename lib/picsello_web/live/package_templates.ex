defmodule PicselloWeb.Live.PackageTemplates do
  ...
  def render(assigns) do
    ...
    <.settings_nav socket={@socket} live_action={@live_action} current_user={@current_user} container_class="sm:pb-0 pb-28">
      <div class={classes("flex flex-col justify-between flex-1 mt-5 sm:flex-row", %{"flex-grow-0" => Enum.any?(@templates) })} {intro(@current_user, "intro_settings_packages")}>
        <div>
          <h1 class="text-2xl font-bold"><span class="intro-settings-packages">Package Templates</span></h1>
