      ...
      <div class={classes("flex flex-col justify-between flex-1 mt-5 sm:flex-row", %{"flex-grow-0" => Enum.any?(@templates) })}>
        <div>
          ...
          <p class="max-w-2xl my-2">
            ...
            <% else %>
              Create reusable pricing and shoot templates to make it easier to manage leads. Looking to learn more about your pricing? <%= link("Checkout out our helpful calculator!", to: Routes.calculator_path(@socket, :index), class: "underline text-blue-planning-300") %>
            <% end %>
          </p>
        </div>
