defmodule PicselloWeb.JobLive.Index do
  ...
  def actions(assigns) do
    ~H"""
      <div class="flex items-center left-3 sm:left-8" data-offset-x="-21" phx-update="ignore" data-placement="bottom-end" phx-hook="Select" id={"manage-job-#{@job.id}"}>
  ...
  defp actions do
    [
      ...
      %{title: "Go to galleries", action: "view-galleries", icon: "galleries"},
      %{title: "Send email", action: "open-compose", icon: "envelope"},
      %{title: "Complete", action: "complete-job", icon: "check"},
