defmodule PicselloWeb.JobLive.Index do
  ...
  def search_sort_bar(assigns) do
    ~H"""
      <div {testid("search_filter_and_sort_bar")} class="flex flex-col px-5 center-container justify-between items-end px-1.5 lg:flex-row mb-10">
        <div class="relative flex w-full lg:w-2/3 mr-2">
        ...
        <.select_dropdown class="w-full md:w-72" type={@type} title={if @type == "job", do: 'Job Status', else: 'Lead Status'} id="status" selected_option={@job_status} options_list={if @type == "job", do: job_status_options(), else: lead_status_options()}/>

        <.select_dropdown class="w-full md:w-72" title={if @type == "job", do: 'Job Type', else: 'Lead Type'} id="type" selected_option={@job_type} options_list={job_type_options(@job_types)}/>

        <.select_dropdown class="w-full md:w-72" sort_direction={@sort_direction} title="Sort" id="sort_by" selected_option={@sort_by} options_list={if @type == "job", do: job_sort_options(), else: lead_sort_options()}/>
