defmodule Picsello.Repo.Migrations.SeedUpdateDefaultPackageTemplates do
  ...
  def change do
    query = from p in "packages", where: not is_nil(p.job_type), select: %{id: p.id, job_type: p.job_type, base_price: p.base_price, base_multiplier: p.base_multiplier}
  ...
  def get_price(%{base_multiplier: base_multiplier, base_price: base_price}, presets_count, index) do
    ...
    amount = Decimal.mult(base_price, base_multiplier) |> Decimal.round(0, :floor) |> Decimal.to_integer()
