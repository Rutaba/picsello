defmodule Picsello.Repo.Migrations.PopulateBookingEventDates do
  ...
  alias Picsello.{Repo, BookingEvent, BookingEvents, BookingEventDate}

  def change do
    from(e in BookingEvent, where: not is_nil(e.old_dates))
    |> Repo.all()
    |> Enum.map(fn booking_event ->
      booking_event_dates = Enum.map(booking_event.old_dates, fn date ->
        BookingEvents.available_times(booking_event, date.date)
      end)
