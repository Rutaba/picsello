defmodule Picsello.Cart.CartProductTest do
  ...
  describe "cart porduct updates" do
    ...
    test "product adding to a cart" do
      gallery = insert(:gallery)
      first_product = confirmed_product()
      order = Cart.place_product(first_product, gallery.id)

      assert first_product.price == order.subtotal_cost

      second_product = confirmed_product("aaaacccccbbb")
      updated_order = Cart.place_product(second_product, gallery.id)

      assert Money.add(first_product.price, second_product.price) == updated_order.subtotal_cost
    end

    test "product removal from a cart" do
      gallery = insert(:gallery)

      first_product = confirmed_product()
      Cart.place_product(first_product, gallery.id)

      second_product = confirmed_product("aaaacccccbbb")
      order = Cart.place_product(second_product, gallery.id)

      {_, reduced_order} = Cart.delete_product(order, first_product.editor_details.editor_id)

      assert reduced_order.subtotal_cost == second_product.price
    end
  end
