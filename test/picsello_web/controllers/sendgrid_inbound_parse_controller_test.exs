defmodule PicselloWeb.SendgridInboundParseControllerTest do
  ...
  setup do
    ...
    %{
      ...
      sendgrid_response: %{
        ...
        "subject" => "Re: Test Subject",
        "text" =>
          "This is my response\r\n\r\nOn Fri, Oct 29, 2021 at 9:50 AM <photographer-notifications@picsello.com> wrote:\r\n\r\n> Test body\r\n>\r\n> Unsubscribe - Unsubscribe Preferences\r\n>\n",
        "to" => "Photography <#{token}@dev-inbox.picsello.com>"
      }
    }
  end

  test "parse the response with attachments", %{
    conn: conn,
    job: job,
    sendgrid_response: sendgrid_response
  } do
    Picsello.PhotoStorageMock
    |> Mox.stub(:insert, fn attachment, _ -> {:ok, attachment} end)

    params =
      sendgrid_response
      |> Map.put("attachments", "2")
      |> Map.put(
        "attachment-info",
        "{\"attachment-1\":{\"filename\":\"attachment-1.jpg\",\"name\":\"attachment-1\",\"type\":\"image/jpeg\",\"size\":1234},\"attachment-2\":{\"filename\":\"attachment-2.png\",\"name\":\"attachment-2\",\"type\":\"image/png\",\"size\":1234}}"
      )
      |> Map.put("attachment-1", %Plug.Upload{
        path: "test/fixtures/files/attachment-1.jpg",
        filename: "attachment-1.jpg",
        content_type: "image/jpeg"
      })
      |> Map.put("attachment-2", %Plug.Upload{
        path: "test/fixtures/files/attachment-2.png",
  ...
  test "parses the response", %{conn: conn, job: job, sendgrid_response: sendgrid_response} do
    params = sendgrid_response
