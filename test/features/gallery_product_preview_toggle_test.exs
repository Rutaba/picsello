  ...
  setup %{gallery: gallery} do
    ...
    [gallery: gallery]
      ...
      test "Toggle disable product in gallery", %{
        ...
        |> find(css("label", text: "Product enabled to sell", visible: false, count: 7, at: 0), fn checkbox -> refute Element.selected?(checkbox) end)
      ...
      test "Toggle disable product and view in client preview", %{
        ...
        |> scroll_to_bottom()
        |> click(css("span", text: "Product enabled to sell", count: 7, at: 0))
        |> find(css("label", "Product enabled to sell", visible: false, count: 7, at: 0), fn checkbox -> refute Element.selected?(checkbox) end)
        |> click(css("span", text: "Show product preview in gallery", count: 6, at: 1))
        |> find(css("label", "Show product preview in gallery", visible: false, count: 6, at: 1), fn checkbox -> refute Element.selected?(checkbox) end)
      ...
      test "Toggle disable product preview and product available for purchase", %{
        ...
        |> find(css("label", "Product enabled to sell", visible: false, count: 7, at: 0), fn checkbox -> refute Element.selected?(checkbox) end)
        |> find(css("label", "Product enabled to sell", visible: false, count: 7, at: 2), fn checkbox -> refute Element.selected?(checkbox) end)
        |> find(css("label", "Product enabled to sell", visible: false, count: 7, at: 3), fn checkbox -> refute Element.selected?(checkbox) end)
