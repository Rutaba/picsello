defmodule Picsello.ClientOrdersTest do
  ...
  setup %{gallery: gallery} do
    ...
    cart_products =
      Enum.map(1..3, fn _ -> build(:ordered_cart_product, %{product_id: whcc_product.whcc_id}) end)

    [
      order:
        insert(:confirmed_order, %{gallery_id: gallery.id, products: cart_products})
        |> Picsello.Cart.set_order_number()
    ]
  end
