defmodule Picsello.ClientBooksEventTest do
  ...
  setup do
    ...
    [
      ...
      template: template,
      event: event,
      event_no_questionnaire: event_no_questionnaire,
      booking_event_url:
      ...
      booking_event_no_questionnaire_url:
        Routes.client_booking_event_path(
          PicselloWeb.Endpoint,
          :show,
          user.organization.slug,
          event_no_questionnaire.id
        )
    ]
  end
  @phone "2015551234"
  ...
  feature "client books event without questionnaire", %{
    session: session,
    photographer: %{organization_id: organization_id},
    event_no_questionnaire: %{id: event_id},
    template: %{id: template_id},
    booking_event_no_questionnaire_url: booking_event_no_questionnaire_url
  } do
    Picsello.MockPayments
    |> Mox.stub(:retrieve_account, fn _, _ ->
      {:ok, %Stripe.Account{charges_enabled: true}}
    ...
    session
    |> visit(booking_event_no_questionnaire_url)
    |> click(link("Book now"))
    |> fill_in(text_field("Your name"), with: " ")
    |> fill_in(text_field("Your email"), with: " ")
    |> fill_in(css("input[type=tel]"), with: " ")
